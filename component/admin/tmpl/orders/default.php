...
$statusLabel=static function(string $status):string {
    $key='COM_DECAROFINANCE_ORDER_STATUS_'.strtoupper($status);
    $label=Text::_($key);
  ...
  <section class="xdecaro-card xdf-form-card">
    ...
    <div class="xdecaro-card__body">
      <form action="<?php echo Route::_('index.php?option=com_decarofinance&task=finance.createOrder'); ?>" method="post" class="xdf-form-grid">
        ...
        <label><?php echo Text::_('COM_DECAROFINANCE_ACCOUNT'); ?><select class="form-select" name="account_id" required><option value="">—</option><?php foreach($this->accounts as $a): ?><option value="<?php echo (int)$a['id']; ?>"><?php echo htmlspecialchars((string)$a['name'].' · '.(string)$a['currency'],ENT_QUOTES,'UTF-8'); ?></option><?php endforeach; ?></select></label>
        ...
        <label><?php echo Text::_('COM_DECAROFINANCE_OWNER_REFERENCE'); ?><select class="form-select" name="owner_organization_uuid"><option value=""><?php echo Text::_('COM_DECAROFINANCE_NO_OWNER'); ?></option><?php foreach($this->organizations as $organization): ?><option value="<?php echo htmlspecialchars((string)$organization['uuid'],ENT_QUOTES,'UTF-8'); ?>"><?php echo htmlspecialchars((string)$organization['name'],ENT_QUOTES,'UTF-8'); ?></option><?php endforeach; ?></select></label>
      ...
      <table class="table table-striped xdf-orders-table">
        ...
        <?php foreach($this->items as $row): $status=(string)$row['status']; $next=(int)$row['approval_count']+1; ?>
          <tr>
            ...
            <td><span class="badge bg-secondary"><?php echo htmlspecialchars($statusLabel($status),ENT_QUOTES,'UTF-8'); ?></span></td>
