...
$s=$this->summary;
$money=static fn($v)=>number_format((float)$v,2,',','.').' '.htmlspecialchars($this->currency,ENT_QUOTES,'UTF-8');
?>
<div class="xdf-finance">
  ...
  <section class="xdecaro-card">
    ...
    <?php foreach($this->categories as $row): ?><tr><td><?php echo htmlspecialchars((string)$row['category'],ENT_QUOTES,'UTF-8'); ?></td><td><?php echo htmlspecialchars((string)$row['direction'],ENT_QUOTES,'UTF-8'); ?></td><td><?php echo (int)$row['movements']; ?></td><td><?php echo $money($row['amount']); ?></td></tr><?php endforeach; ?>
    </tbody></table></div>
  </section>
  ...
  <section class="xdecaro-card">
    ...
    <?php foreach($this->budgetUsage as $row): ?><tr><td><?php echo htmlspecialchars((string)$row['budget_title'],ENT_QUOTES,'UTF-8'); ?></td><td><?php echo htmlspecialchars((string)$row['title'],ENT_QUOTES,'UTF-8'); ?></td><td><?php echo $money($row['planned_amount']); ?></td><td><?php echo $money($row['realized_amount']); ?></td><td><?php echo $money($row['committed_amount']); ?></td><td><?php echo $money($row['available_amount']); ?></td></tr><?php endforeach; ?>
    </tbody></table></div>
  </section>
</div>
