...
$lineTypeLabel=static function(string $type):string {
    $key='COM_DECAROFINANCE_LINE_TYPE_'.strtoupper($type);
    $label=Text::_($key);
  ...
  <section class="xdecaro-card xdf-form-card">
    ...
    <div class="xdecaro-card__body">
      <form action="<?php echo Route::_('index.php?option=com_decarofinance&task=finance.createStatement'); ?>" method="post" class="xdf-form-grid">
        ...
        <label><?php echo Text::_('COM_DECAROFINANCE_OWNER_REFERENCE'); ?>
          <select class="form-select" name="owner_organization_uuid">
            <option value=""><?php echo Text::_('COM_DECAROFINANCE_NO_OWNER'); ?></option>
            <?php foreach($this->organizations as $organization): ?><option value="<?php echo htmlspecialchars((string)$organization['uuid'],ENT_QUOTES,'UTF-8'); ?>"><?php echo htmlspecialchars((string)$organization['name'],ENT_QUOTES,'UTF-8'); ?></option><?php endforeach; ?>
          </select>
        </label>
        <div class="xdf-span-2"><div class="form-text"><?php echo Text::_('COM_DECAROFINANCE_STATEMENT_LINKS_AUTOMATIC_HELP'); ?></div></div>
    ...
    <section class="xdecaro-card xdf-statement">
      <div class="xdecaro-card__header xdf-statement-head">
        <div>
          ...
          <div class="xdf-muted">
            <?php echo htmlspecialchars($typeLabel((string)$statement['statement_type']),ENT_QUOTES,'UTF-8'); ?>
            · <?php echo htmlspecialchars((string)($statement['period_start']??''),ENT_QUOTES,'UTF-8'); ?> → <?php echo htmlspecialchars((string)($statement['period_end']??''),ENT_QUOTES,'UTF-8'); ?>
            <?php if(trim((string)($statement['owner_label']??''))!==''): ?> · <?php echo htmlspecialchars((string)$statement['owner_label'],ENT_QUOTES,'UTF-8'); ?><?php endif; ?>
          </div>
        </div>
      ...
      <div class="xdecaro-card__body">
          ...
          <table class="table table-sm">
            ...
            <?php foreach($lines as $line): ?>
              <tr>
                ...
                <td><?php echo htmlspecialchars($lineTypeLabel((string)$line['line_type']),ENT_QUOTES,'UTF-8'); ?></td>
        ...
        <div class="xdf-row-actions xdf-statement-actions">
          <?php if($status==='draft' && $this->canReconcile): ?><form action="<?php echo Route::_('index.php?option=com_decarofinance&task=finance.finaliseStatement'); ?>" method="post"><input type="hidden" name="statement_id" value="<?php echo $id; ?>"><button class="btn btn-outline-primary" type="submit"><?php echo Text::_('COM_DECAROFINANCE_FINALISE'); ?></button><?php echo HTMLHelper::_('form.token'); ?></form><?php endif; ?>
          <?php if($status==='finalised' && $this->canApprove): ?><form action="<?php echo Route::_('index.php?option=com_decarofinance&task=finance.approveStatement'); ?>" method="post"><input type="hidden" name="statement_id" value="<?php echo $id; ?>"><button class="btn btn-success" type="submit"><?php echo Text::_('COM_DECAROFINANCE_APPROVE'); ?></button><?php echo HTMLHelper::_('form.token'); ?></form><?php endif; ?>
        </div>
      </div>
    </section>
  <?php endforeach; ?>
